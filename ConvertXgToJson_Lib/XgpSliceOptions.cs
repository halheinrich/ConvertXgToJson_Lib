...
/// changes nothing.
///
/// <para><b>Player-name overrides.</b> A non-null name rewrites that
/// player's two match-header fields — the Unicode field
...
public sealed record XgpSliceOptions
{
    ...
    /// <summary>
    /// Preset for anonymized export: both players renamed to the neutral
    /// "Player 1" / "Player 2". This instance is the single source of what
    /// "anonymized" means — consumers wire an anonymize toggle to it rather
    /// than encoding their own replacement names.
    /// </summary>
    ...
    private static string? ValidateName(string? value)
    {
        if (value is not null && string.IsNullOrWhiteSpace(value))
            throw new ArgumentException(
                "A player-name override must be non-empty; use null to keep the source name.",
                nameof(value));
        return value;
    }
}
